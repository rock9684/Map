...
LatLon getIntersectionPosition(unsigned intersectionID) {
    return projection((streetsDB.roads())[streetsDB.intersection(intersectionID)].latlon);
}
...
LatLon getStreetSegmentCurvePoint(unsigned streetSegmentID, unsigned idx) {
    ...
    return projection(G[e].curvePoints.at(idx));
}
...
LatLon getPointOfInterestPosition(unsigned pointOfInterestID) {
    return projection(streetsDB.poi(pointOfInterestID).pos());
...
LatLon getFeaturePoint(unsigned featureID, unsigned idx) {
    return projection(streetsDB.feature(featureID).point(idx));
}

LatLon projection(LatLon point) {
    if (projected) {
        point.prelat = point.lat;
        point.prelon = point.lon;
        point.lon = point.lon * DEG_TO_RAD * Cos;
        point.lat = point.lat * DEG_TO_RAD;
    }
