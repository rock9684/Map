...
struct LatLon {
    ...
    float prelat = std::numeric_limits<float>::quiet_NaN();
    float prelon = std::numeric_limits<float>::quiet_NaN();
    ...
    explicit LatLon(int, float ilat, float ilon) : prelat(ilat), prelon(ilon) {
    }
